...
install_v2ray(){
	...
	sed -i 's#ps": ".*"#ps": "google cloud"#g' $(cat v2raypath)
	...
	temppath="/SoftDown"
	cat /root/temp.json |jq '.inbounds[0].streamSettings.wsSettings.path="'${temppath}'"' |jq '.inbounds[0].streamSettings.wsSettings.headers.Host="www.bilibili.com"' |jq '.inbounds[0].settings.clients[0].id="c14fc873-7062-4956-ad43-7409dbb76acc"' > /etc/v2ray/config.json
